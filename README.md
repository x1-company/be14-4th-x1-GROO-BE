...
### __“감정을 심고, 감정으로 자라는 나만의 숲.”__
텍스트 기반 감정 기록을 시각적으로 표현한, 개인화된 감성 다이어리 서비스 그루 (Groo)
...
<details>
  ...
  |이름|회고|
  |------|---|
  |박지원|**이번 프로젝트에서 맡은 역할**<br>- OpenAI API를 활용한 감정 분석 기능 구현<br>- 감정 일기의 등록, 조회, 수정, 삭제 등 일기 관리 백엔드 구현<br>- 감정 분석 결과로 얻은 아이템을 보여주는 화면의 프론트 개발 및 백엔드 연동<br><br>**잘한 점**<br>- 팀원들과의 활발한 소통과 협업을 통해 개발 효율을 높일 수 있었습니다.<br>- 예상치 못한 문제 상황에서도 서로를 격려하고 배려하는 분위기 속에서 프로젝트를 잘 마무리할 수 있었습니다.<br><br>**아쉬운 점**<br>- 백엔드 개발 중간에 기능이 계속 추가되면서 계획했던 프론트엔드 작업을 충분히 할 시간이 부족했습니다.<br>- 개발 일정이 촉박해지면서 일부 구현은 빠르게 처리해야 했고 이는 완성도나 디테일에 아쉬움을 남겼습니다.<br><br>**앞으로의 개선 방향**<br>- 초기 요구사항 정의를 좀 더 명확히 하고 개발 중간에 변경사항이 생기더라도 그 영향을 최소화할 수 있도록 구조를 유연하게 잡는 것이 중요하겠다는 교훈을 얻었습니다.<br>- 이번 프로젝트를 통해 기능 설계, API 연동, 팀워크 등 다양한 측면에서 성장할 수 있었고 이 경험을 발판 삼아 더 나은 개발자가 되도록 노력하겠습니다.|
  |신민경| 이번 프로젝트는 감정을 기록하고 정원으로 표현하는 감성 다이어리 서비스였습니다. 주제는 자유롭게 정할 수 있었는데, 예전에 국립현대미술관 외부에 설치된 정원 꾸미기 스크린이 떠올라 이를 바탕으로 아이디어를 제안했고, 팀원들도 이에 공감해 주며 주제가 구체화되었습니다.<br><br>‘일기를 쓰고, 그 감정에 따라 숲을 꾸민다’는 콘셉트를 시각적으로 구현하기 위해 UI 설계에 많은 고민을 쏟았습니다. 백엔드를 주로 공부해왔기 때문에 프론트엔드 구현은 다소 낯설었지만, 팀원들과 수차례 회의를 거쳐 의견을 조율한 끝에 만족스러운 결과물을 완성할 수 있었습니다.<br><br>이번 프로젝트를 통해 가장 크게 느낀 점은 요구사항 정의의 중요성이었습니다. 요구사항을 충분히 정리하지 않은 채 개발을 시작하면서, 중간에 다시 정의 단계로 되돌아가야 했고, 이에 따라 코드 수정은 물론 모델링 설계도 일부 변경해야 했습니다. 또한 일정 관리가 미흡했던 탓에 구현하고자 했던 기능 중 일부를 제외하게 된 점도 아쉬움으로 남습니다.<br><br>기술적으로는 MyBatis와 JPA를 함께 활용하여 백엔드 로직을 구현하면서, 데이터베이스 설계와 트랜잭션 처리 등에 대한 이해도를 크게 높일 수 있었습니다. 또한 테스트 과정에서는 백엔드에서의 예외 처리와 에러 응답 설계 덕분에 프론트엔드 개발 시 오류를 빠르게 파악하고 수정할 수 있었던 점이 좋았습니다.<br><br>2주라는 짧은 기간 안에 기획부터 개발, 배포까지 모든 과정을 마치는 것은 쉽지 않았지만, 팀원들의 헌신과 협력이 있었기에 끝낼 수 있었다고 생각합니다. 밤낮없이 더 나은 서비스를 만들기 위해 함께 고민하고, 서로를 배려하며 달려온 팀원들 덕분에 중간에 지칠 때마다 다시 힘을 얻을 수 있었고, 그 결과 완성도 높은 결과물을 만들 수 있었습니다.<br><br>마지막으로, 프로젝트 시연 날 다른 팀으로부터 좋은 피드백을 받을 수 있었던 건, 전적으로 모든 팀원이 각자의 역할에 최선을 다해 몰두해준 덕분이라고 생각합니다. 팀원들에게 진심으로 감사드리며, 모두 수고 많았다는 말을 꼭 전하고 싶습니다. |
  ...
  |황수민|2주라는 짧은 기간 동안 기획부터 프론트엔드, 백엔드, 배포까지 전 과정을 직접 경험해볼 수 있어 정말 많은 성장을 느낄 수 있었던 시간이었습니다. 초반에 기획에 많은 시간을 투자했지만, 요구사항은 계속해서 바뀌었고 그에 따라 디자인과 모델링도 여러 번 수정하게 되면서 요구사항 정의가 얼마나 중요한지 다시 한 번 깨달았습니다. 프론트엔드 개발은 아직 익숙하지 않지만, 이번 프로젝트를 계기로 three.js, babylon.js 같은 새로운 기술을 공부하고 직접 적용해보면서 프론트 쪽에 대한 흥미도 생겼습니다. 또한, OpenAI를 활용한 감정 일기 분석 로직을 팀원과 함께 구현하면서 REST API와 백엔드 흐름에 대해 더 깊이 이해하게 되었고, 덕분에 개발자로서 한층 더 성장할 수 있었다고 생각합니다. 아직 부족한 점도 많지만, 예전보다 개발 속도도 빨라지고 자신감도 붙은 제 모습을 보면서 스스로도 많이 놀랐습니다. 무엇보다, 이 프로젝트는 팀원들과 함께했기에 가능했던 결과라고 생각합니다. 혼자였다면 절대 2주 안에 이 정도 서비스를 완성하지 못했을 거라 생각합니다. 이번 경험을 통해 협업의 중요성과 팀워크의 힘을 다시 한 번 느낄 수 있었고, 앞으로도 함께하는 사람들의 소중함을 잊지 않고 개발해 나가고 싶습니다.  |
...
- **잘한 점**
  - 힘든 일정 속에서도 서로 웃음을 잃지 않고, 각자의 역할에 책임감을 가지고 끝까지 협력해 나갔다는 점이 인상적이었습니다.<br>
    팀워크와 긍정적인 분위기가 프로젝트를 끝까지 끌고 갈 수 있었던 큰 원동력이 되었습니다.

- **아쉬운 점**
  - 작업 중 파일 충돌을 피하기 위해 패키지를 나눴지만, 공통으로 사용할 수 있는 파일까지 분리되면서 오히려 불필요하게 파일 수가 많아졌습니다.<br>
    향후에는 패키지 구조를 정리하고, 공통 파일은 적절히 모아 관리할 수 있도록 리팩토링이 필요합니다.

- **앞으로의 개선 방향**
  - placement 테이블에 scale과 z_index 컬럼을 추가하여, 아이템 배치 시 크기 조절 및 앞뒤 순서를 조정할 수 있도록 개선할 예정입니다.<br>
    이를 통해 사용자가 아이템을 더욱 자유롭고 직관적으로 커스터마이징할 수 있게 하여, 배치의 표현력을 높일 수 있습니다.

### 🧑🏻‍💻 이준규
- **프로젝트에서 맡은 부분**
  - (BE) 메인 화면에서 필요한 숲id에 따른 상세 조회 기능 구현
  - (BE) 숲의 공개 상태 변경 기능 구현
  - (FE) 로그인 이후 보여질 메인 화면 구현
  - (FE) 날씨 컴포넌트 구현
  - (FE) AI 감정분석 결과에 따른 날씨 컴포넌트 호출 로직 구현

- **잘한 점**
  - 메인 화면에 해당하는 복잡한 조회 기능을 구현했으며, 여러 테이블을 조인해야 하는 대용량 데이터를 효율적으로 처리함.
  - 서비스의 핵심 기능인 감정 분석 결과에 따른 날씨 반영 기능을 오류 없이 잘 만들었음.
  - 날씨 컴포넌트를 구현하는 과정에서 외부 라이브러리로 표현하기 어려운 날씨들도 있었지만, 포기하지 않고 사진을 연속적으로 보여주거나, 속도·밝기·투명도 등을 조절하는 다양한 방법을 활용해 UI 측면에서 사용자의 흥미를 유도할 수 있도록 노력함.
    

- **아쉬운 점**
  - 주로 조회 기능을 담당하여 만들어서 JPA 활용을 거의 해보지 못 했다.
  - SNS 로그인 기능을 만들어보고 싶었지만 시간이 부족하여 실패했다. 추후 리팩토링과정에서 추가해볼 예정이다.
  - Fine-Tuning을 할 수 있는 능력이 있었다면 조금 더 정확도 높은 감정 분석 결과를 도출해 낼 수 있었을 텐데 이를 하지 못 해 감정 분석의 정확도가 조금 아쉬웠다.
  - 기능 개발을 한 후 수정하는 일이 많았음. 코드를 작성하기 전 서비스에 대한 이해도와 테이블 구조를 좀 더 깊게 생각하고  개발에 들어가야할 필요성을 느꼈다.
    

- **앞으로의 개선 방향**
  - 개발 하기에 앞서 서비스에 대한 이해도를 높이고 테이블 구조 파악을 철저하게 해야겠다.
  - 프론트엔드 공부를 좀 더 해서 프로젝트 진행함에 있어 팀원들에게 도움을 주고싶다.

### 👩🏻‍💻 황수민
- **프로젝트에서 맡은 부분**
  - Figma를 활용한 서비스 전반의 UI/UX 기획 및 설계
  - OpenAI API를 활용한 일기 감정 분석 기반 랜덤 아이템 추천 기능 개발
  - 일기 수정 기능 및 날짜·기간 기반 감정 일기 조회 기능 구현
  - LottieFiles 기반의 감정 분석 진행 중 로딩 애니메이션 기능 구현
  - 프론트엔드 반응형 사이드바 컴포넌트 개발 및 Vue.js 기반 라우팅 설정
  - 날짜 기반 일기 작성 및 감정 분석 화면 UI 개발
   
- **잘한 점**
  -  AI 감정 분석 API의 지연 시간을 고려해 Lottie 로딩 애니메이션을 추가, 사용자 이탈을 방지하고 UX 향상 유도
  -  Figma 컴포넌트 설계를 통해 디자인 일관성과 재사용성 확보
  -  일기 등록 → 감정 분석 → 아이템 추천 및 저장까지의 흐름을 단일 API 플로우로 구현, 사용자 중심의 자연스러운 경험 제공

- **아쉬운 점**
  - 요구사항과 테스트케이스를 구체적으로 작성하지 않아 Figma 설계 수정이 잦고 팀원 간 UI 합의에 시간이 소요됨
  - 기획에 시간이 많이 소요되어 일부 기능은 구현하지 못해 아쉬움이 남음
  - 예외 처리와 단위 테스트가 부족해 안정성 측면에서 아쉬움
  - 재사용 가능한 컴포넌트 설계를 놓쳐 프론트엔드 코드 중복 발생

- **앞으로의 개선 방향**
  - 마감기한에 촉박하게 개발을 하지 않고, 시간적 여유를 두고 여유롭게 개발하는 사람이 되고싶다.
  - 팀원들의 PR 리뷰를 보다 꼼꼼하게 검토할 것
  -  모델링 부분에서 좀 더 적극적으로 참여하여, 데이터 정합성 및 효율을 고려한 모델링 설계를 주도해 보고싶다.
